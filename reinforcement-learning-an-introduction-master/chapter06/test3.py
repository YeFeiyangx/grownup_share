...
def step(state, action_2agv, goal):
    ...
    for agv in range(2):
        ...
        if action == ACTION_UP:
            ...
            if agv == 0:
                _next_state = (i, max(j - 1, PASS_SIDE[agv][0] ))
            elif agv == 1:
                _next_state = (i, max(j - 1, PASS_SIDE[agv][0] ))
                
        elif action == ACTION_RIGHT:
            if agv == 0:
                _next_state = (i, min(j + 1, PASS_SIDE[agv][1] ))
            elif agv == 1:
                _next_state = (i, min(j + 1, PASS_SIDE[agv][1] ))
    ...
    # elif next_state[0][0] == next_state[1]
    ...
    while len(goal) != 0:
        ...
        for agv in range(2):
            q_value[state[0], state[1], agv, action] += step_size * (
                    reward + GAMMA * np.max(q_value[next_state[0], next_state[1], agv, :]) -
...
def figure_6_4():
    ...
    for r in tqdm(range(runs)):
        ...
        for i in range(0, episodes):
            if n_times >= 0 and n_times < 4000:
                epsilon = 0.9
            elif n_times >= 4000 and n_times < 8000:
                epsilon = 0.7
            elif n_times >= 8000 and n_times < 12000:
                epsilon = 0.5
            elif n_times >= 12000 and n_times < 16000:
                epsilon = 0.3
            else:
                epsilon = 0.05

            rewards_q_learning[i] += q_learning(q_q_learning, epsilon)
            n_times += 1
