...
def q_learning(q_value, fst, fbst, epsilon, gamma, print_time, step_size):
    state = START
    rewards_global = 0
    ...
    for r in tqdm(range(runs)):
        for i in range(0, episodes):
            ...
            if n_times >= 0 and n_times < 0.2*M:
                ...
                epsilon = 0.05
                # alpha = 0.3
            reward_global = q_learning(q_q_learning, fst, fbst, epsilon, gamma, n_times, step_size=alpha)
            ...
            if sum_reward_global >= rewards_q_learning.max():
                ...
                if sum_reward_global > his_reward_global + 2:
                    ...
                    name = '2agv_grw_bze_gqt_kg_' + str(int(sum_reward_global)) + '.npy'
                    np.save("D:\\gitShare\\alg_pic\\" + name, q_q_learning)
            n_times += 1
    np.save("D:\\gitShare\\alg_pic\\2agv_grw_bze_gqt_kg.npy", q_q_learning)
